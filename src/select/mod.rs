        ...
        if let Some(current) = current {
            ...
            for c in current {
                match c {
                    ...
                    Value::String(vec) => {
                        let index = abs_index(index as isize, vec.len());
                        let ch = vec.chars().nth(index);
                        if let Some(ch) = ch {
                            let string = ch.to_string();
                            std::mem::replace(owned, Some(Value::String(string)));
                        }
                    }
                    _ => {}
                }
...
impl<'a, 'b> Selector<'a, 'b> {
    ...
    pub fn select_owned(&mut self) -> Result<Vec<Value>, JsonPathError> {
        self._select()?;

        match &self.owned {
            Some(o) => return Ok(vec![o.clone()]),
            None => {}
        };

        match &self.current {
            Some(r) => Ok(r.iter().map(|&v| v.clone()).collect()),
            _ => Err(JsonPathError::EmptyValue),
        }
    }
